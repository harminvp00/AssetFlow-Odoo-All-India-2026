...
import { Paperclip, Plus, FileText, ShieldAlert } from 'lucide-react';
...
export default function AttachmentsPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.name) {
      toast.error('Please enter an attachment name.');
      return;
    }
    ...
    <div className="space-y-6">
      ...
      {attachments.length === 0 ? (
        <div className="p-8 text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm">
          ...
          <h3 className="text-lg font-bold text-slate-800 dark:text-white">No Attachments Found</h3>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
            Get started by uploading a new invoice, warranty, or proof document.
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800/40 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
                <th className="px-6 py-4">Attachment Name</th>
                <th className="px-6 py-4">Created At</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800 text-sm">
              {attachments.map((att) => (
                <tr key={att.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 text-slate-800 dark:text-slate-200">
                  <td className="px-6 py-4 font-bold flex items-center gap-2">
                    <FileText className="h-4 w-4 text-slate-400" />
                    {att.name}
                  </td>
                  <td className="px-6 py-4">{new Date(att.createdAt || Date.now()).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">Document Name *</label>
                <input type="text" name="name" placeholder="e.g. MacBook Pro Invoice" value={formData.name} onChange={handleInputChange} className="w-full px-3 py-2 border border-slate-200 dark:border-slate-800 rounded-lg dark:bg-slate-950 text-slate-950 dark:text-white" required />
              </div>
