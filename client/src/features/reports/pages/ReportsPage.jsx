...
import { BarChart3, Plus, FileSpreadsheet, ShieldAlert } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      {reports.length === 0 ? (
        <div className="p-8 text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm">
          ...
          <h3 className="text-lg font-bold text-slate-800 dark:text-white">No Reports Generated</h3>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
            Get started by generating an asset registry or allocation audit report.
          </p>
        </div>
      ) : (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm overflow-hidden">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800 text-sm">
              {reports.map((report) => (
                <tr key={report.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 text-slate-800 dark:text-slate-200">
                  <td className="px-6 py-4 font-bold flex items-center gap-2">
                    <FileSpreadsheet className="h-4 w-4 text-slate-400" />
                    {report.name}
                  </td>
                  <td className="px-6 py-4">{new Date(report.createdAt || Date.now()).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">Report Title / Name *</label>
                <input type="text" name="name" placeholder="e.g. Asset Cost Allocation Report Q2" value={formData.name} onChange={handleInputChange} className="w-full px-3 py-2 border border-slate-200 dark:border-slate-800 rounded-lg dark:bg-slate-950 text-slate-950 dark:text-white" required />
              </div>
